...
### 개선됨 🔧
- **시각 기준 명시화**: 모든 시간 관련 기능이 클라이언트(사용자 컴퓨터) 로컬 시각을 기준으로 작동함을 명시
  - 날짜/시간 포맷팅 함수에 클라이언트 시간대 명시적 지정
  - 관찰 기록, AI 요약, 데이터 내보내기 등 모든 타임스탬프가 클라이언트 로컬 시각 기준
  - `getCurrentDate()` 함수를 `toISOString()` 대신 로컬 시각 기준으로 개선
  - 코드 주석에 "클라이언트 로컬 시각 기준" 명시
- **문서화**: README에 시각 기준 설명 추가

### 변경됨 🔄
- 날짜/시간 포맷팅 함수에 `timeZone` 옵션 추가하여 일관성 보장
- 로컬 스토리지 키 생성 시 `getCurrentDate()` 함수 사용으로 통일
