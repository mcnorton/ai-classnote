...
├── prompt.txt              # AI 분석용 프롬프트 템플릿 (커스터마이징 가능)
├── js/
...
`prompt.txt` 파일을 텍스트 에디터로 열어서 AI에게 전달되는 프롬프트를 자유롭게 수정할 수 있습니다.
...
- 프롬프트 파일 로드 실패 시 기본 프롬프트가 자동으로 사용됩니다
- 변경사항은 페이지 새로고침 후 즉시 적용됩니다

### 🔄 프롬프트 캐싱

- 프롬프트는 첫 로드 후 메모리에 캐시되어 성능이 최적화됩니다
- 프롬프트를 수정한 후에는 페이지를 새로고침하여 변경사항을 적용하세요
- 개발자 도구에서 `promptTemplate = null`을 실행하여 캐시를 수동으로 초기화할 수 있습니다

## 🛡️ 보안 및 개인정보

- **로컬 실행**: 모든 데이터는 브라우저의 로컬 스토리지에 저장
- **외부 연결**: Gemini API 호출 시에만 인터넷 연결 필요
- **데이터 암호화**: 로컬 스토리지는 브라우저가 자동으로 관리
- **API 키**: 세션 스토리지에 임시 저장 (페이지 새로고침 시까지)
